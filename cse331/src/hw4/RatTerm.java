...
public final class RatTerm {
	...
	/**
	 * @param c
	 *            the coefficient of the RatTerm to be constructed.
	 * @param e
	 *            the exponent of the RatTerm to be constructed.
	 * @requires c != null
	...
	public double eval(double d) {
		...
		return Math.pow(d, (double) this.expt) * this.coeff.doubleValue();
	}
	...
	public RatTerm differentiate() {
		RatNum exp = new RatNum(this.expt);
		return new RatTerm(this.coeff.mul(exp), this.expt - 1);
	}
	...
	public RatTerm antiDifferentiate() {
		RatNum exp = new RatNum(this.expt + 1);
		return new RatTerm(this.coeff.div(exp), this.expt + 1);
	}
	 ...
	 *         <p>
	 *
	 *         The string for a non-zero, non-NaN RatTerm is in the form "C*x^E"
	 *         where C is a valid string representation of a RatNum (see
	 *         {@link hw4.RatNum}'s toString method) and E is an integer.
	 *         UNLESS: (1) the exponent E is zero, in which case T takes the
	 *         form "C" (2) the exponent E is one, in which case T takes the
	 *         form "C*x" (3) the coefficient C is one, in which case T takes
	 *         the form "x^E" or "x" (if E is one) or "1" (if E is zero).
	 *         <p>

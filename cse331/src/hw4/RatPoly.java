...
public final class RatPoly {
	 ...
	 */
	public RatPoly add(RatPoly p) { //return NaN RatPoly or RatPoly that isNaN
		List<RatTerm> newTerms = p.copy();
		for (RatTerm rt : this.terms) {
	...
	public RatPoly div(RatPoly p) {
		if (p.isNaN() || p.terms.size() == 0 || this.isNaN()) {
			return NaN;
		}
	...
	public RatPoly differentiate() {
		...
		ArrayList<RatTerm> lst = this.copy();
		for (int i = 0; i < lst.size(); i++) {
			RatTerm rt = lst.get(i);
			if (rt.getExpt() == 0) {
				lst.remove(i);
	...
	public RatPoly antiDifferentiate(RatNum integrationConstant) {
		...
		ArrayList<RatTerm> lst = this.copy();
		for (int i = 0; i < lst.size(); i++) {
			lst.set(i, lst.get(i).antiDifferentiate());
		}
